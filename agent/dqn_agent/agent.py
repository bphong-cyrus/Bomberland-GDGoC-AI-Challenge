...
This is the ONLY file the evaluation server calls. It is intentionally tiny and
imports nothing heavier than numpy + torch (+ the sibling `model.py`). Training
lives in `train.py` and is never imported here.

Flat submission zip should contain:
    agent.py        (this file)
    model.py        (shared encoder / network / safety planner)
...
try:                                  # flat submission (sibling file on sys.path)
...
if __name__ == "__main__":
    # Tiny self-test / timing benchmark (no training here — see train.py).
    import time
    from model import compute_danger  # noqa: F401

    base = Path(__file__).parent
    if not (base / "model.pt").exists() and not (base / "model.pth").exists():
        print("No weights found - train first with:  python -m agent.dqn_agent.train")
    else:
        agent = Agent(0)
